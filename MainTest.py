...
while True:
    mylcd.lcd_display_string("Stress Detection", 1)
    mylcd.lcd_display_string("     System     ", 2)
    time.sleep(3)
    mylcd.lcd_clear()
    mylcd.lcd_display_string("White for Start", 1)
    mylcd.lcd_display_string("Red for Reset", 2)
    print("Waiting for Button 1 press...")
    button1.wait_for_press()
    mylcd.lcd_clear()
    mylcd.lcd_display_string(" Wait Prosses ", 1)
    mylcd.lcd_display_string(" Until Finish ", 2)
    time.sleep(2)
    ...
    if __name__ == "__main__":
        ...
        dispatcher_instance.map("/muse/eeg", eeg_handler)
    ...
    # Delay to avoid accidental multiple button presses
    sleep(1)